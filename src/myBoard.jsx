...
function MyBoard() {
  ...
  return (
    ...
    >
      <Typography variant="h6" fontWeight="600" mb={2} textAlign="left" sx={{
            display: "flex",
          ...
          <TableHead>
            <TableRow sx={{ backgroundColor: "#baa9a9ff", }}>
              {headers.map((header, idx) => (
                <TableCell key={idx} sx={{ fontWeight: 600, minWidth: isMobile ? 120 : "auto" }}>
                  {header}
                </TableCell>
              ))}
            </TableRow>
          ...
          <TableBody>
            {val.map((row, index) => (
              <TableRow key={index} sx={{ backgroundColor: "#f5f5f5" }}>
                ...
                <TableCell><Box sx={{ display: "flex", flexDirection: "column", gap: 0.5 }}>
  ...
  </Box></TableCell><tableCell><Typography sx={{pt:"9px"}}>{row.Id}</Typography><Typography variant="body2">{row.passport}</Typography><Typography variant="body2">{row.loaction}</Typography></tableCell>
